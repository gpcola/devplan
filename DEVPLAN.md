...
The DEVPLAN Tracker Pack is a standalone, dependency-free tracker that can be installed into any software repository to keep a human-readable plan (`DEVPLAN.md`) and machine-readable state (`DEVPLAN-STATE.json`) aligned.
...
Any pull request that starts, completes, blocks, defers, supersedes, adds, removes, or materially changes tracked work must update `DEVPLAN-STATE.json` in the same pull request. Completed work must include evidence.
## Newly tracked work
